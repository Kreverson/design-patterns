...
public class TwitterNotification implements NotificationStrategy {
    private final Logger logger = LoggerFactory.getLogger(TwitterNotification.class);

    @Override
    public void sendNotification(Notification notification) {
        //logic
        logger.info(MessageFormat.format("Twitter message sended to {0}", notification.getDestination()));
    }
}